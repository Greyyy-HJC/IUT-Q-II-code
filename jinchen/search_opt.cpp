...
// Optimized version for Ubuntu 22.04
// g++ -std=c++17 -march=native -O3 -ftree-vectorize search.cpp -pthread -lgmp -o search
...
std::vector<std::string> search_for_r_s_and_t_ge_70(int r, int s, int l) {
    ...
    // 为每个线程 tid 分配一段 x1：[1+2*tid, actual_bound]，步长 2*num_threads
    for (int tid = 0; tid < num_threads; ++tid) {
        futures.push_back(thread_pool.enqueue([&, tid]() {
            for (unsigned long x1 = 1 + 2 * tid; x1 <= actual_bound; x1 += 2 * num_threads) {
                // —— 进度更新 & 打印 —— 
                unsigned long p = ++progress;
                if (p % progress_interval == 0 || p == total) {
                    std::lock_guard<std::mutex> lock(cout_mutex);
                    std::cout << "r=" << r << " s=" << s 
                            << " t>=70 Progress: " 
                ...
                // —— 原来的过滤 & 计算逻辑 —— 
                if (x1 % 2 == 0 || x1 % l == 0) continue;
                auto result = check_x1_r(x1, r, l, s);
                if (!result.empty()) {
                    std::lock_guard<std::mutex> lock(solutions_mutex);
                    solutions.insert(solutions.end(), result.begin(), result.end());
                }
                if (r != s) {
                    auto result2 = check_x1_r(x1, s, l, r);
                    if (!result2.empty()) {
                        std::lock_guard<std::mutex> lock(solutions_mutex);
                        solutions.insert(solutions.end(), result2.begin(), result2.end());
                    }
                }
            }
        }));
    ...
    metrics.wall_time = search_duration.count();
    metrics.cpu_time = search_cpu_time;
...
std::vector<std::string> search_for_r_s_and_big_t(int r, int s, int t, int l) {
    ...
    for (int tid = 0; tid < num_threads; ++tid) {
        futures.push_back(thread_pool.enqueue([&, tid]() {
            const size_t start_idx = tid * tasks_per_thread;
            const size_t end_idx = std::min(start_idx + tasks_per_thread, tasks.size());
            
            for (size_t i = start_idx; i < end_idx; ++i) {
                unsigned long z1 = tasks[i].first;
                unsigned long x1 = tasks[i].second;
                
                // —— 进度更新 & 打印 —— 
                unsigned long p = ++progress;
                if (p % progress_interval == 0 || p == total) {
                    std::lock_guard<std::mutex> lock(cout_mutex);
                    std::cout << "r=" << r << " s=" << s << " t=" << t 
                            << " Progress: " 
                            << std::fixed << std::setprecision(2)
                            << (100.0 * p / total) << "%" 
                            << std::endl;
                ...
                // —— 原来的过滤 & 计算逻辑 —— 
                if (x1 % 2 == 0 || x1 % l == 0 || z1 % l == 0 || gcd(x1, z1) != 1) continue;
                ...
                if (r != s) {
                    auto result2 = check_x1_r_y1_s(x1, s, z1, t, l, r);
                    if (!result2.empty()) {
                        std::lock_guard<std::mutex> lock(solutions_mutex);
                        solutions.insert(solutions.end(), result2.begin(), result2.end());
                    }
...
std::vector<std::string> search_for_r_s_and_t(int r, int s, int t, int l) {
    ...
    // 并行动态 schedule(dynamic, 1) 调度
    #pragma omp parallel for schedule(dynamic, 1)
    for (unsigned long z1 = 1; z1 <= real_bound; z1 += 2) {
      unsigned long x1_bound = real_bound / z1;
      for (unsigned long x1 = 1; x1 <= x1_bound; x1 += 2) {
        // 进度更新 & 打印
        unsigned long p = ++progress;
        if (p % progress_interval == 0 || p == total) {
          #pragma omp critical
          {
            std::cout << "r=" << r << " s=" << s << " t=" << t
                      ...
                      << (100.0 * p / total) << "%" << std::endl;
          }
        }
        // 过滤条件
        if (x1 % 2 == 0 || x1 % l == 0 || z1 % l == 0 || gcd(x1, z1) != 1)
          continue;
        // 核心计算
        auto result = check_x1_r_y1_s(x1, r, z1, t, l, s);
        if (!result.empty()) {
          #pragma omp critical
          solutions.insert(solutions.end(), result.begin(), result.end());
        }
        ...
        if (s != t) {
          auto result3 = check_x1_r_y1_s(x1, r, z1, t, l, s);
          if (!result3.empty()) {
            #pragma omp critical
            solutions.insert(solutions.end(), result3.begin(), result3.end());
          }
        }
      }
    }
...
// Task 2: Search for 4 <= r <= s <= t (note that we have 4 <= r,s < 70) where r + s >= 14
std::vector<std::string> task_2() {
    std::vector<std::string> solutions;
...
// Task 3: Case of (r,s) = (4,9)
std::vector<std::string> task_3() {
    return get_solutions_for_fixed_r_s_and_t_ge_u0(4, 9, 9);
}
    /*
    Performance Statistics:
    ------------------------
...
// Task 4: Case of (r,s) = (5,8)
std::vector<std::string> task_4() {
    return get_solutions_for_fixed_r_s_and_t_ge_u0(5, 8, 8);
}
    /*
    Performance Statistics:
    ------------------------
...
// Task 5: Case of (r,s) = (6,7)
std::vector<std::string> task_5() {
    return get_solutions_for_fixed_r_s_and_t_ge_u0(6, 7, 7);
}
    /* 
    Performance Statistics:
    ------------------------
